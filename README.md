...
| **Marine Map** (center) | Real map of Hood Canal + Puget Sound with whale sighting pins from the last 7 days (recent ones pulse; the three newest are numbered and listed in a "Latest sightings" box), clock, LIVE count | [Acartia](https://acartia.io) open sightings feed (Orca Network / Whale Alert data), CARTO tiles |
| **Weather & Tides** (right‑top) | Current conditions, 4‑step forecast strip, interpolated tide curve with next high/low and "now" dot, tide direction/rate | OpenWeatherMap (`NEXT_PUBLIC_WEATHER_API_KEY`), NOAA CO‑OPS station **9445478 Union, Hood Canal** |
| **Our Events** (left‑top) | The household calendar (bravefoote@gmail): next entries; matching local events are marked GOING; a trash button on each row deletes the event from Google Calendar (needs the service account) | Google Calendar (service account) · Hermes `ourEvents` · ICS feed |
| **Calendar view** (fades in over the live panels when you hover Our Events; stays while you're using it, fades out 3 s after the mouse goes idle; `?view=calendar` pins it on) | Google Calendar's own embed UI for the household calendar (must be public to show on the kiosk) | Google Calendar embed (`NEXT_PUBLIC_OUR_CALENDAR_ID`) |
